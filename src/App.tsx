...
function App() {
  ...
  async function updateExpirationDate(productId: string, value: string) {
    ...
    const { error } = await supabase
      .from('products')
      .update({ expiration_date: value || null })
      .eq('id', productId)
    ...
    setSelectedProduct((current) =>
      current?.product_id === productId
        ? { ...current, expiration_date: value || null }
        : current
    )
  ...
  async function createNewProduct() {
    ...
    if (!newName.trim() || !newSupplier.trim() || !newCategory.trim()) {
      setErrorMessage('Nome, fornitore e categoria sono obbligatori')
      return
    }

    const supplierId = await getOrCreateNamedRecord(
      'suppliers',
      newSupplier.trim()
    )
    ...
    const categoryId = await getOrCreateNamedRecord(
      'categories',
      newCategory.trim()
    )
  ...
  function changeOrderQuantity(productId: string, value: string) {
    setOrderItems((current) => {
      ...
      return [...current, { product_id: productId, quantity: value }]
    })
  }

  function getOrderQuantity(productId: string) {
    return (
      orderItems.find((item) => item.product_id === productId)
        ?.quantity ?? ''
    )
  }
  ...
  function exportOrderExcel() {
    ...
    const rows = orderItems
      .map((orderItem) => {
        ...
        if (!product || quantity === null || quantity <= 0) return null
    ...
    const total = rows.reduce(
      (sum, row) => sum + Number(row['Spesa prevista (€)']),
      0
    )
    ...
    const worksheet = utils.json_to_sheet([
      ...rows,
      {},
      { Prodotto: 'TOTALE', 'Spesa prevista (€)': total },
    ])
  ...
  async function confirmExcelImport() {
    ...
    if (importMode === 'replace') {
      ...
      if (typed !== 'SOSTITUISCI') {
        setErrorMessage('Sostituzione annullata: conferma non valida')
        return
      }
    } else {
      if (
    ...
    setSuccessMessage(
      `Importazione completata: ${result?.created ?? 0} nuovi, ` +
        `${result?.updated ?? 0} aggiornati, ` +
        `${result?.adjustments ?? 0} rettifiche, ` +
        `${result?.deactivated ?? 0} disattivati.`
    )
  }

  const selectedLoadProduct = inventory.find(
    (product) => product.product_id === loadProductId
  )
  ...
  const orderTotal = useMemo(
    () =>
      orderItems.reduce((sum, orderItem) => {
        ...
        if (!product || quantity === null || quantity <= 0) return sum

        return sum + quantity * getPurchasePrice(product)
      }, 0),
  ...
  const tiedCapital = useMemo(
    () =>
      allProducts.reduce(
        (sum, product) =>
          sum +
          product.availability * getPurchasePrice(product),
        0
      ),
    [allProducts]
  )
  ...
  const inventorySaleValue = useMemo(
    () =>
      allProducts.reduce(
        (sum, product) =>
          sum +
          product.availability * Number(product.sale_price ?? 0),
        0
      ),
    [allProducts]
  )
  ...
  const completedSalesRevenue = useMemo(
    () =>
      sales
        .filter((sale) => sale.status === 'COMPLETATA')
        .reduce(
  ...
  const purchaseOutflow = useMemo(
    () =>
      movements
        .filter((movement) =>
          ['INVENTARIO_INIZIALE', 'CARICO'].includes(
        ...
        .reduce(
          (sum, movement) =>
            sum +
            movement.quantity * Number(movement.unit_cost ?? 0),
          0
        ),
    [movements]
  )
    ...
    return (
      <div className="login-page">
        <div className="login-card">
          ...
          <h1>Gramsci Warehouse Management</h1>
          <p>Accedi per gestire il magazzino.</p>
  ...
  return (
    <div className="app">
      ...
      <main className="main-content">
        {page === 'inventory' && (
          <section className="inventory-print-area">
            <div className="print-only print-inventory-header">
              <h1>Gramsci Warehouse Management</h1>
              <h2>Inventario</h2>
              <p>Stampato il {new Date().toLocaleString('it-IT')}</p>
            </div>
            ...
            <div className="no-print">
              ...
              <div className="inventory-actions">
                ...
                <label className="btn btn-upload">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (file) await handleExcelUpload(file, 'update')
                ...
                <label className="btn btn-danger-solid">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      const file = e.target.files?.[0]
                      if (file) await handleExcelUpload(file, 'replace')
                      e.currentTarget.value = ''
                    }}
                  />
                </label>
              ...
              <div className="summary-grid">
                ...
                <SummaryCard
                  ...
                  value={
                    inventory.filter((item) => item.availability <= 0)
                      .length
                  }
                />
              </div>
              ...
              {excelRows.length > 0 && (
                <ImportPreview
                  ...
                  productsToDeactivate={productsToDeactivate}
            ...
            <InventoryTable
              ...
              onExpirationChange={updateExpirationDate}
            />
          </section>
        )}
        ...
        {page === 'load' && (
          <>
            ...
            <div className="panel form-panel">
              ...
              >
                {showNewProduct ? 'Chiudi nuovo prodotto' : '+ Nuovo prodotto'}
              </button>
            ...
            <div className="panel">
              <div className="table-wrapper">
                <table>
                  ...
                  <tbody>
                    {inventory.map((product) => {
                      const quantityText = getOrderQuantity(
                        product.product_id
                      )
                      ...
                      return (
                        <tr key={product.product_id}>
                          ...
                          <td>{formatQuantity(product.availability, product.unit_type)}</td>
                          ...
                          <td>{rowTotal > 0 ? formatPrice(rowTotal) : '-'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div className="order-footer">
                <div>
                  <span>Spesa totale prevista</span>
        ...
        {page === 'statistics' && (
          <>
            ...
            <div className="stats-grid">
              ...
              <SummaryCard label="Margine potenziale" value={formatPrice(inventorySaleValue - tiedCapital)} />
              ...
              <SummaryCard label="Cashflow cumulato" value={formatPrice(completedSalesRevenue - purchaseOutflow)} />
            </div>

            <div className="panel">
              <h3>Cashflow cumulato</h3>
                ...
                <div key={sale.id} className="sale-card">
                  ...
                  <div className="sale-items">
                    {sale.sale_items?.map((item) => (
                      <div key={item.id}>
                        <span>
                          {getProductName(item.products)} × {item.quantity}
                        </span>
                  ...
                  <tbody>
                    {movements.map((movement) => (
                      <tr key={movement.id}>
                        ...
                        <td><strong>{getProductName(movement.products)}</strong></td>
                        <td><MovementBadge type={movement.movement_type} /></td>
                        <td>{movement.quantity}</td>
                        <td>{movement.stock_before}</td>
                        <td>{movement.stock_after}</td>
                        <td>{formatPrice(movement.unit_cost)}</td>
            ...
            {!inactiveProducts.length ? (
              ...
              <div className="inactive-products-grid">
                {inactiveProducts.map((product) => (
                  <div className="inactive-product-card" key={product.product_id}>
                    <div className="inactive-product-header">
                      <h3>{product.name}</h3>
                      <span className="badge badge-neutral">DISATTIVATO</span>
                    </div>
        ...
        {errorMessage && (
          <div className="alert alert-error no-print">{errorMessage}</div>
        )}

        {successMessage && (
          <div className="alert alert-success no-print">{successMessage}</div>
        )}
      </main>
  ...
  return (
    <div
      className={`excel-import-panel ${
        mode === 'replace' ? 'excel-import-danger' : ''
      }`}
    >
      ...
      <div className="import-summary-grid">
        ...
        {mode === 'replace' && (
          <SummaryCard
            ...
            value={productsToDeactivate.length}
          />
        )}
      </div>

      {changedRows.length === 0 &&
        (mode !== 'replace' || productsToDeactivate.length === 0) && (
          <div className="no-changes-box">
            Il file coincide con l’inventario attuale: non risultano modifiche.
          </div>
        )}
        ...
        <>
          ...
          <div className="changes-list">
            {changedRows.map((row, index) => (
              <div className="change-card" key={`${row.product_id ?? row.name}-${index}`}>
      ...
      {mode === 'replace' && productsToDeactivate.length > 0 && (
        <div className="deactivate-preview">
          <h4>Prodotti assenti dall’Excel</h4>
          <p>
            Questi prodotti verranno portati a giacenza zero tramite rettifica
            e poi disattivati:
          </p>

          <div className="deactivate-tags">
            {productsToDeactivate.map((product) => (
              <span key={product.product_id} className="badge badge-danger">
                {product.name}
              </span>
            ))}
          </div>
            ...
            {inventory.map((item) => (
              <tr key={item.product_id}>
                <td>
                  ...
                  <span className="print-only">{item.name}</span>
                </td>

                <td>{item.supplier ?? '-'}</td>
                <td>{item.category ?? '-'}</td>
                <td>{formatPrice(item.normal_price)}</td>
                <td>{formatPrice(item.confidential_price)}</td>
                ...
                <td>
                  <input
                    ...
                    onChange={(e) =>
                      onExpirationChange(item.product_id, e.target.value)
                    }
                  />
                ...
                <td>
                  <ExpirationBadge
                    days={getDaysToExpiration(item.expiration_date)}
                  />
                </td>
              </tr>
            ))}
    ...
    <div className="modal-overlay no-print" onMouseDown={onClose}>
      <div className="product-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="product-modal-header">
          <div>
            <span className="modal-eyebrow">Gestione prodotto</span>
            <h2>{product.name}</h2>
          </div>
        ...
        <div className="product-modal-grid">
          ...
          <ProductDetail
            label="Disponibilità"
            value={formatQuantity(product.availability, product.unit_type)}
          />
        ...
        <div className="modal-expiration">
          ...
          <input
            ...
            onChange={(e) =>
              onExpirationChange(product.product_id, e.target.value)
            }
          />
        </div>
        ...
        <div className="modal-danger-zone">
          <div>
            <strong>Disattiva prodotto</strong>
            <p>
              Il prodotto verrà nascosto dalle operazioni correnti, ma vendite
              e movimenti storici resteranno conservati.
            </p>
          </div>
...
function CashflowChart({ points }: { points: CashflowPoint[] }) {
  if (!points.length) {
    return <div className="empty-state">Nessun dato disponibile.</div>
  }
  ...
  return (
    <button
      className={`nav-button ${
        page === pageName ? 'nav-button-active' : ''
      }`}
...
function ExpirationBadge({ days }: { days: number | null }) {
  ...
  if (days <= 30) return <span className="badge badge-danger">{days} gg</span>
  if (days <= 90) return <span className="badge badge-warning">{days} gg</span>
  return <span className="badge badge-success">{days} gg</span>
}
